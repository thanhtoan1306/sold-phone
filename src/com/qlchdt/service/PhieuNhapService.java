...
import com.qlchdt.dao.PhieuNhapDao;
...
public class PhieuNhapService {
   PhieuNhapDao phieuNhapDao = new PhieuNhapDao();
    ...
    public boolean add(PhieuNhap pn) {
        Boolean ok = phieuNhapDao.add(pn);
        if (ok) {
            dspn.add(pn);
        }
        return ok;
    ...
    public Boolean updateTongTien(String _mapn, float _tongTien) {
        Boolean success = phieuNhapDao.updateTongTien(_mapn, _tongTien);
        if (success) {
            for (PhieuNhap pn : dspn) {
    ...
    public boolean delete(String ma) {
        Boolean ok = phieuNhapDao.delete(ma);
        if (ok) {
